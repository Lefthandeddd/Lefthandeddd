...
# дело в том, что ты вверху переопределяешь словарь distances на словарь с ответами, а я тебе шпредлагал сделать функцию которая будет вызываться в процессе итерации
# и заполнять НОВЫЙ словарь с ответами. Не переопределять изначальные переменные стоит тогда, когда тебе это реально нужно в ходу выполнения задачи